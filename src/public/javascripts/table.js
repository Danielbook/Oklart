define([
  ...
  Table.prototype.drawTimeTable = function(time, idx) {
    ...
    for(var i = 0; i < timespan; i++){
      ...
      "<td style='width:20%' class='toggleable t row"+i+"part' onclick=updateLocation("+idx+",'t',"+i+");_table.highlightColumn('t',"+i+"); ><span class='minTemp'>" + this._data[idx].mintimeseries[time].t + "°</span> "+this._data[idx].timeseries[time].t+"° <span class='maxTemp'>" + this._data[idx].maxtimeseries[time].t + "°</span></td>" +
      ...
      "<td style='width:20%' class='toggleable pit row"+i+"parpit' onclick=updateLocation("+idx+",'pit',"+i+");_table.highlightColumn('pit',"+i+"); >"+this.snowOrRain(this._data[idx].timeseries[time])+"-"+ this._data[idx].maxtimeseries[time].pit +" mm</td>" +
      "<tr>");
    }
  }
